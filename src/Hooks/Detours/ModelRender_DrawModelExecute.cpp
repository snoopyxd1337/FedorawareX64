...
void DrawBT(void* ecx, CBaseEntity* pEntity, const DrawModelState_t& pState, const ModelRenderInfo_t& pInfo, matrix3x4* pBoneToWorld);
...
{
	FDBG_SCOPE_SLOW("Hook.ModelRender_DrawModelExecute", 8000);

	if (I::EngineClient->IsTakingScreenshot() && Vars::Visuals::CleanScreenshots.Value)
	{
		FDBG_SKIP("ModelRender.Features", "clean screenshot active");
		return Hook.Original<FN>()(ecx, pState, pInfo, pBoneToWorld);
	}

	CBaseEntity* pEntity = I::ClientEntityList->GetClientEntity(pInfo.m_nEntIndex);

	FDBG_CALL("Feature.Backtrack.DrawBT", DrawBT(ecx, pEntity, pState, pInfo, pBoneToWorld));


	if (!F::Glow.m_bRendering)
	{
		bool dmeRendered = false;
		FDBG_CALL("Feature.DMEChams.Render", dmeRendered = F::DMEChams.Render(pState, pInfo, pBoneToWorld));
		if (dmeRendered) { return; }
	}
	else
	{
		FDBG_SKIP("Feature.DMEChams.Render", "Glow is rendering");
	}

	FDBG_CALL("ModelRender.Original", Hook.Original<FN>()(ecx, pState, pInfo, pBoneToWorld));
}
...
void DrawBT(void* ecx, CBaseEntity* pEntity, const DrawModelState_t& pState, const ModelRenderInfo_t& pInfo,
			matrix3x4* pBoneToWorld)
{
	...
	if (Vars::Backtrack::BtChams::LastOnly.Value)
	{
		...
		if (vLastRec)
		{
			...
			OriginalFn(ecx, pState, pInfo, (matrix3x4*)(&vLastRec->BoneMatrix));
		}
	}
	else
	{
		for (size_t i = 0; i < vRecords->size(); i++)
		{
			...
			OriginalFn(ecx, pState, pInfo, (matrix3x4*)(&pRecord.BoneMatrix));
		}
	}
