...
MAKE_HOOK(NetChannel_SendDatagram, S::NetChannel_SendDatagram(), int, __fastcall, INetChannel* netChannel, bf_write* datagram)
{
	if (!netChannel || datagram) {
		return Hook.Original<FN>()(netChannel, datagram);
	}
	...
	const int original = Hook.Original<FN>()(netChannel, datagram);
