...
MAKE_HOOK(C_BaseEntity_SetAbsVelocity, S::CBaseEntity_SetAbsVelocity(), void, __fastcall, void* ecx, const Vec3& vecAbsVelocity)
{
	static uintptr_t dwSetAbsVelocityCall = g_Pattern.Find("client.dll", "0F 28 74 24 ? 8B D6");

	if (uintptr_t(_ReturnAddress()) == dwSetAbsVelocityCall)
	{
				...
				if (flSimTimeDelta > 0.0f)
				{
					...
					if (!(nCurFlags & FL_ONGROUND) && !(nOldFlags & FL_ONGROUND))
					{
						...
						if (bCorrected)
						{
							...
							Hook.Original<FN>()(ecx, vNewVelocity);
							return;
						}
					}
				}
	...
	Hook.Original<FN>()(ecx, vecAbsVelocity);
}
