...
#include "../Auto/AutoGlobal/AutoGlobal.h"
...
namespace S
{
	MAKE_SIGNATURE(UTIL_PlayerByIndex, SERVER_DLL, "48 83 EC ? 8B D1 85 C9 7E ? 48 8B 05", 0x0);
	MAKE_SIGNATURE(CBaseAnimating_DrawServerHitboxes, SERVER_DLL, "44 88 44 24 ? 53 48 81 EC", 0x0);
	MAKE_SIGNATURE(RenderLine, ENGINE_DLL, "48 89 5C 24 ? 48 89 74 24 ? 44 89 44 24", 0x0);
	MAKE_SIGNATURE(RenderBox, ENGINE_DLL, "48 83 EC ? 8B 84 24 ? ? ? ? 4D 8B D8", 0x0);
	MAKE_SIGNATURE(RenderWireframeBox, ENGINE_DLL, "48 89 5C 24 ? 48 89 74 24 ? 48 89 7C 24 ? 55 41 54 41 55 41 56 41 57 48 8D AC 24 ? ? ? ? 48 81 EC ? ? ? ? 49 8B F9", 0x0);
}

void CVisuals::Draw()
{
	FDBG_SCOPE("Feature.Visuals.DrawBody");

	if (const auto pLocal = g_EntityCache.GetLocal())
	{
		FDBG_CALL("Feature.Visuals.DrawAntiAim", DrawAntiAim(pLocal));
		FDBG_CALL("Feature.Visuals.DrawTickbaseInfo", DrawTickbaseInfo(pLocal));
		FDBG_CALL("Feature.Visuals.DrawAimbotFOV", DrawAimbotFOV(pLocal));
		FDBG_CALL("Feature.Visuals.ScopeLines", ScopeLines(pLocal));
		FDBG_CALL("Feature.Visuals.DrawDebugInfo", DrawDebugInfo(pLocal));
		FDBG_CALL("Feature.Visuals.DrawAimbotDebugIndicator", DrawAimbotDebugIndicator(pLocal));
		FDBG_CALL("Feature.Visuals.DrawOnScreenConditions", DrawOnScreenConditions(pLocal));
		FDBG_CALL("Feature.Visuals.DrawOnScreenPing", DrawOnScreenPing(pLocal));
	}
	else
	{
		FDBG_SKIP("Feature.Visuals.LocalDraws", "local entity null");
	}

	FDBG_CALL("Feature.Visuals.DrawNoSpreadIndicator", DrawNoSpreadIndicator());
	FDBG_CALL("Feature.Visuals.DrawServerHitboxes", DrawServerHitboxes());
	FDBG_CALL("Feature.Visuals.DrawPredictionLine", DrawPredictionLine());
	FDBG_CALL("Feature.Visuals.PickupTimers", PickupTimers());
	FDBG_CALL("Feature.Visuals.SetVisionFlags", SetVisionFlags());
}
...
void CVisuals::DrawNoSpreadIndicator() 
{
	...
	if (!Vars::NoSpread::Indicator.Value || !Vars::NoSpread::Hitscan.Value || F::NoSpread.ServerTime <= 0.0f)
	{
		return;
	}
...
{
	if (!Vars::Visuals::SkyboxChanger.Value || !I::Cvar)
	{
		return;
	}

	const auto svSkyname = I::Cvar->FindVar("sv_skyname");
	if (!svSkyname)
	{
		return;
	}
	...
	const char* desiredSky = nullptr;
	if (Vars::Skybox::SkyboxNum.Value == 0)
	{
		if (Vars::Misc::BypassPure.Value && !Vars::Skybox::SkyboxName.Value.empty())
		{
			desiredSky = Vars::Skybox::SkyboxName.Value.c_str();
		}
		else
		{
			desiredSky = svSkyname->GetString();
		}
	}
	else
	{
		constexpr int skyboxCount = sizeof(skybNames) / sizeof(skybNames[0]);
		if (Vars::Skybox::SkyboxNum.Value > 0 && Vars::Skybox::SkyboxNum.Value < skyboxCount)
		{
			desiredSky = skybNames[Vars::Skybox::SkyboxNum.Value];
		}
	}

	if (desiredSky && *desiredSky && std::string_view(svSkyname->GetString()) != desiredSky)
	{
		svSkyname->SetValue(desiredSky);
	}
}
...
void CVisuals::ThirdPerson(CViewSetup* pView)
{
	if (const auto& pLocal = g_EntityCache.GetLocal())
	{
		...
		{
			Vec3 vAngles = F::AntiAim.vRealAngles;
...
namespace
{
	...
	const char* WeaponTypeStr(EWeaponType type)
	{
		switch (type)
		{
		case EWeaponType::HITSCAN: return "hitscan";
		case EWeaponType::PROJECTILE: return "projectile";
		case EWeaponType::MELEE: return "melee";
...
void CVisuals::DrawDebugInfo(CBaseEntity* pLocal)
{
	...
	if (Vars::Debug::DebugInfo.Value)
	{
		...
		const auto pWeapon = g_EntityCache.GetWeapon();
		g_Draw.String(menuFont, xoffset, yoffset += 20, {119, 255, 225, 255}, ALIGN_DEFAULT, "Aimbot");
		g_Draw.String(menuFont, xoffset, yoffset += 15, {255, 255, 255, 255}, ALIGN_DEFAULT, "active: %s", BoolStr(Vars::Aimbot::Global::Active.Value));
		g_Draw.String(menuFont, xoffset, yoffset += 15, {255, 255, 255, 255}, ALIGN_DEFAULT, "key down: %s", BoolStr(F::AimbotGlobal.IsKeyDown()));
		g_Draw.String(menuFont, xoffset, yoffset += 15, {255, 255, 255, 255}, ALIGN_DEFAULT, "weapon type: %s", WeaponTypeStr(G::CurWeaponType));
		g_Draw.String(menuFont, xoffset, yoffset += 15, {255, 255, 255, 255}, ALIGN_DEFAULT, "move stop: %s  fast accel: %s", BoolStr(F::Misc.bMovementStopped), BoolStr(F::Misc.bFastAccel));
		g_Draw.String(menuFont, xoffset, yoffset += 15, {255, 255, 255, 255}, ALIGN_DEFAULT, "can attack: %s / %s", BoolStr(G::WeaponCanAttack), BoolStr(G::WeaponCanSecondaryAttack));
		g_Draw.String(menuFont, xoffset, yoffset += 15, {255, 255, 255, 255}, ALIGN_DEFAULT, "headshot: %s  frozen: %s", BoolStr(G::WeaponCanHeadShot), BoolStr(G::Frozen));
		g_Draw.String(menuFont, xoffset, yoffset += 15, {255, 255, 255, 255}, ALIGN_DEFAULT, "target idx: %d  cmd: %d", G::CurrentTargetIdx, G::CurrentUserCmd ? G::CurrentUserCmd->command_number : 0);
		g_Draw.String(menuFont, xoffset, yoffset += 15, {255, 255, 255, 255}, ALIGN_DEFAULT, "stage: %s", G::AimbotDebugStage.c_str());
		g_Draw.String(menuFont, xoffset, yoffset += 15, {255, 210, 120, 255}, ALIGN_DEFAULT, "reason: %s", G::AimbotDebugReason.c_str());

		g_Draw.String(menuFont, xoffset, yoffset += 20, {119, 255, 225, 255}, ALIGN_DEFAULT, "Triggerbot");
		g_Draw.String(menuFont, xoffset, yoffset += 15, {255, 255, 255, 255}, ALIGN_DEFAULT, "active: %s", BoolStr(Vars::Triggerbot::Global::Active.Value));
		g_Draw.String(menuFont, xoffset, yoffset += 15, {255, 255, 255, 255}, ALIGN_DEFAULT, "key down: %s", BoolStr(F::AutoGlobal.IsKeyDown()));
		g_Draw.String(menuFont, xoffset, yoffset += 15, {255, 255, 255, 255}, ALIGN_DEFAULT, "should shift: %s", BoolStr(G::ShouldShift));
		g_Draw.String(menuFont, xoffset, yoffset += 15, {255, 255, 255, 255}, ALIGN_DEFAULT, "weapon id: %d  item def: %d", pWeapon ? pWeapon->GetWeaponID() : -1, pWeapon ? pWeapon->GetItemDefIndex() : -1);
		g_Draw.String(menuFont, xoffset, yoffset += 15, {255, 210, 120, 255}, ALIGN_DEFAULT, "reason: %s", G::TriggerDebugReason.c_str());
	}
}

void CVisuals::DrawAimbotDebugIndicator(CBaseEntity* pLocal)
{
	if (!Vars::Debug::AimbotIndicator.Value) { return; }
	if (!pLocal || !pLocal->IsAlive() || pLocal->IsAGhost()) { return; }
	if (Vars::Visuals::CleanScreenshots.Value && I::EngineClient->IsTakingScreenshot()) { return; }

	const auto& font = g_Draw.GetFont(FONT_INDICATORS);
	const int x = Vars::Debug::AimbotIndicatorPos.Value.c;
	int currentY = Vars::Debug::AimbotIndicatorPos.Value.y;

	g_Draw.String(font, x, currentY += 15, { 119, 255, 225, 255 }, ALIGN_CENTERHORIZONTAL, "AIMBOT");
	g_Draw.String(font, x, currentY += 15, Vars::Aimbot::Global::Active.Value ? Color_t{ 153, 232, 0, 255 } : Color_t{ 255, 95, 95, 255 }, ALIGN_CENTERHORIZONTAL, "Active: %s", BoolStr(Vars::Aimbot::Global::Active.Value));
	g_Draw.String(font, x, currentY += 15, { 255, 255, 255, 255 }, ALIGN_CENTERHORIZONTAL, "Key %s | Type %s", BoolStr(F::AimbotGlobal.IsKeyDown()), WeaponTypeStr(G::CurWeaponType));
	g_Draw.String(font, x, currentY += 15, { 255, 255, 255, 255 }, ALIGN_CENTERHORIZONTAL, "Can %s/%s | Stop %s", BoolStr(G::WeaponCanAttack), BoolStr(G::WeaponCanSecondaryAttack), BoolStr(F::Misc.bMovementStopped));
	g_Draw.String(font, x, currentY += 15, { 255, 255, 255, 255 }, ALIGN_CENTERHORIZONTAL, "FastAccel %s | Target %d", BoolStr(F::Misc.bFastAccel), G::CurrentTargetIdx);
	g_Draw.String(font, x, currentY += 15, { 255, 255, 255, 255 }, ALIGN_CENTERHORIZONTAL, "IN_ATTACK %s | Send %s", BoolStr(G::DebugAttackButton), BoolStr(G::DebugSendPacket));
	g_Draw.String(font, x, currentY += 15, { 255, 255, 255, 255 }, ALIGN_CENTERHORIZONTAL, "Silent %s | HSilent %s", BoolStr(G::SilentTime), BoolStr(G::HitscanSilentActive));
	g_Draw.String(font, x, currentY += 15, { 255, 255, 255, 255 }, ALIGN_CENTERHORIZONTAL, "cur %.3f np %.3f", G::DebugCurTime, G::DebugNextPrimaryAttack);
	g_Draw.String(font, x, currentY += 15, { 255, 255, 255, 255 }, ALIGN_CENTERHORIZONTAL, "na %.3f ns %.3f", G::DebugNextAttack, G::DebugNextSecondaryAttack);
	g_Draw.String(font, x, currentY += 15, { 255, 210, 120, 255 }, ALIGN_CENTERHORIZONTAL, "%s :: %s", G::AimbotDebugStage.c_str(), G::AimbotDebugReason.c_str());

	currentY += 10;
	g_Draw.String(font, x, currentY += 15, { 119, 255, 225, 255 }, ALIGN_CENTERHORIZONTAL, "TRIGGER");
	g_Draw.String(font, x, currentY += 15, Vars::Triggerbot::Global::Active.Value ? Color_t{ 153, 232, 0, 255 } : Color_t{ 255, 95, 95, 255 }, ALIGN_CENTERHORIZONTAL, "Active: %s", BoolStr(Vars::Triggerbot::Global::Active.Value));
	g_Draw.String(font, x, currentY += 15, { 255, 255, 255, 255 }, ALIGN_CENTERHORIZONTAL, "Key %s | Shift %s", BoolStr(F::AutoGlobal.IsKeyDown()), BoolStr(G::ShouldShift));
	g_Draw.String(font, x, currentY += 15, { 255, 210, 120, 255 }, ALIGN_CENTERHORIZONTAL, "%s", G::TriggerDebugReason.c_str());
}
...
void CVisuals::DrawServerHitboxes()
{
	...
	{
		using PlayerByIndex_t = void* (*)(int);
		using DrawServerHitboxes_t = void(__fastcall*)(CBaseEntity*, float, bool);
		static auto PlayerByIndex = S::UTIL_PlayerByIndex.As<PlayerByIndex_t>();
		static auto DrawServerHitboxes = S::CBaseAnimating_DrawServerHitboxes.As<DrawServerHitboxes_t>();

		if (!PlayerByIndex || !DrawServerHitboxes)
		{
			return;
		}
		...
		{
			if (const auto pServerPlayer = reinterpret_cast<CBaseEntity*>(PlayerByIndex(I::EngineClient->GetLocalPlayer())))
			{
				DrawServerHitboxes(pServerPlayer, 0.f, true);
			}
		}
	}
}
...
void CVisuals::RenderBox(const Vec3& vPos, const Vec3& vMins, const Vec3& vMaxs, const Vec3& vOrientation, Color_t cEdge, Color_t cFace)
{
	if (cFace.a)
	{
		...
		fnRenderBox(vPos, vOrientation, vMins, vMaxs, cFace, false, false);
	}
	if (cEdge.a)
	{
		using FN = void(__cdecl*)(const Vec3&, const Vec3&, const Vec3&, const Vec3&, Color_t, bool);
		static auto fnRenderWireframeBox = S::RenderWireframeBox.As<FN>();
		if (!fnRenderWireframeBox)
		{
			return;
		}
		fnRenderWireframeBox(vPos, vOrientation, vMins, vMaxs, cEdge, false);
	}
}
...
void CVisuals::DrawProjectileTracer(CBaseEntity* pLocal, const Vec3& position)
{
	...
	if (const auto pWeapon = pLocal->GetActiveWeapon())
	{
		...
		pWeapon->GetAttachment(iAttachment, shootPos);
	}
	else
	{
		shootPos = pLocal->GetShootPos();
	}
...
void CVisuals::StoreMaterialHandles()
{
	...
	for (MaterialHandle_t h = I::MaterialSystem->First(); h != I::MaterialSystem->Invalid(); h = I::MaterialSystem->Next(h))
	{
		if (const auto& pMaterial = I::MaterialSystem->Get(h))
		{
			...
			const auto groupName = pMaterial->GetTextureGroupName();
			const auto materialName = pMaterial->GetName();
			if (!groupName || !materialName)
			{
				continue;
			}

			auto sGroup = std::string_view(groupName);

			data.Group = sGroup;

			auto sName = std::string_view(materialName);
