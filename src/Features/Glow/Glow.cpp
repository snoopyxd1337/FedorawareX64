...
namespace
{
	void ReleaseMaterial(IMaterial*& pMaterial)
	{
		if (!pMaterial) { return; }

		F::DMEChams.UnregisterMaterial(pMaterial);
		pMaterial->DecrementReferenceCount();
		pMaterial->DeleteIfUnreferenced();
		pMaterial = nullptr;
	}

	void ReleaseTexture(ITexture*& pTexture, bool bDeleteIfUnreferenced)
	{
		if (!pTexture) { return; }
...
void CGlowEffect::Init()
{
	if (!m_pRtFullFrame)
	{
		...
		if (m_pRtFullFrame) { m_pRtFullFrame->IncrementReferenceCount(); }
	}

	if (!m_pRtQuarterSize1)
	{
		m_pRtQuarterSize1 = I::MaterialSystem->FindTexture("_rt_SmallFB1", TEXTURE_GROUP_RENDER_TARGET);
		if (m_pRtQuarterSize1) { m_pRtQuarterSize1->IncrementReferenceCount(); }
	}

	if (m_pRtFullFrame && !m_pRenderBuffer1)
	{
		...
		if (m_pRenderBuffer1) { m_pRenderBuffer1->IncrementReferenceCount(); }
	}

	if (m_pRtFullFrame && !m_pRenderBuffer2)
	{
		m_pRenderBuffer2 = I::MaterialSystem->CreateNamedRenderTargetTextureEx(
			...
			RT_SIZE_LITERAL, IMAGE_FORMAT_RGB888, MATERIAL_RT_DEPTH_SHARED,
			TEXTUREFLAGS_CLAMPS | TEXTUREFLAGS_CLAMPT | TEXTUREFLAGS_EIGHTBITALPHA, CREATERENDERTARGETFLAGS_HDR);
		if (m_pRenderBuffer2) { m_pRenderBuffer2->IncrementReferenceCount(); }
	}

	if (!m_pMatGlowColor || !m_pMatBlurX || !m_pMatBlurXwf || !m_pMatBlurY || !m_pMatBlurYwf || !m_pMatHaloAddToScreen)
	{
		CreateMaterials();
	}
}
...
void CGlowEffect::CreateMaterials(){
	...
	if (m_pMatGlowColor)
	{
		...
		F::DMEChams.RegisterMaterial(m_pMatGlowColor);
	}
...
void CGlowEffect::DeleteMaterials(){
	ReleaseMaterial(m_pMatGlowColor);
	ReleaseMaterial(m_pMatBlurXwf);
	ReleaseMaterial(m_pMatBlurX);
	ReleaseMaterial(m_pMatBlurYwf);
	ReleaseMaterial(m_pMatBlurY);
	ReleaseMaterial(m_pMatHaloAddToScreen);
...
void CGlowEffect::Unload()
{
	DeleteMaterials();
	ReleaseTexture(m_pRenderBuffer1, true);
	ReleaseTexture(m_pRenderBuffer2, true);
	ReleaseTexture(m_pRtFullFrame, false);
	ReleaseTexture(m_pRtQuarterSize1, false);
}
...
void CGlowEffect::Render()
{
	...
	if (const auto& pLocal = g_EntityCache.GetLocal())
	{
		...
		if (!m_pMatGlowColor || !m_pMatBlurX || !m_pMatBlurY || !m_pMatHaloAddToScreen
			|| !m_pRenderBuffer1 || !m_pRenderBuffer2 || !m_pRtQuarterSize1)
		{
			Init();
			if (!m_pMatGlowColor || !m_pMatBlurX || !m_pMatBlurY || !m_pMatHaloAddToScreen
				|| !m_pRenderBuffer1 || !m_pRenderBuffer2 || !m_pRtQuarterSize1)
