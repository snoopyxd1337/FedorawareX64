...
inline void CFakelag::Prediction(const EHANDLE& nOldGroundInt, const int nOldFlags) {
	PreserveBlastJump(nOldGroundInt, nOldFlags);
}

inline void CFakelag::PreserveBlastJump(const EHANDLE& nOldGroundInt, const int nOldFlags) {
	if (bPreservingBlast) { return; }
	CBaseEntity* pLocal = g_EntityCache.GetLocal();
	const bool bVar = Vars::Misc::CL_Move::RetainBlastJump.Value;
	const bool bPlayerReady = pLocal->IsAlive() && pLocal->IsPlayer() && pLocal->OnSolid() && (!nOldGroundInt.Get() || !(nOldFlags & FL_ONGROUND));
...
void CFakelag::Run(CUserCmd* pCmd, bool* pSendPacket, const EHANDLE& nOldGround, const int nOldFlags) {
