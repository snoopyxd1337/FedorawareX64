...
bool CSpectatorList::GetSpectators(CBaseEntity* pLocal)
{
	...
	{
		CBaseEntity* pObservedPlayer = I::ClientEntityList->GetClientEntityFromHandleSafe(
...
void CSpectatorList::DrawDefault()
{
	...
	if (const auto& pLocal = g_EntityCache.GetLocal())
	{
		...
		const int h = nFontTall * static_cast<int>(Spectators.size());
		...
		for (size_t n = 0; n < Spectators.size(); n++)
		{
			...
			y = SpecListY + SpecListTitleBarH + (nFontTall * static_cast<int>(n));
