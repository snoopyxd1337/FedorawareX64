...
{
	if (!I::EngineClient || !I::ClientEntityList || !I::EngineClient->IsInGame() || !g_EntityCache.GetLocal())
	{
		return;
	}
	...
	for (int n = 1; n < I::ClientEntityList->GetHighestEntityIndex(); n++)
	{
		...
		if (!pEntity->IsAlive() || pEntity->GetDormant() || !pEntity->IsInValidTeam()) { continue; }
		...
		if (IsSimulationReliable(pEntity))
		{
			...
			const float flLastRecorded = mRecords[pEntity].empty() ? flSimTime : mRecords[pEntity].front().flSimTime;
			const float flDeltaRecorded = flNextSimTime - flLastRecorded;
