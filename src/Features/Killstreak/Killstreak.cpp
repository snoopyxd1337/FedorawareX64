...
void CKillstreaker::ApplyKillstreak()
{
	if (Vars::Misc::KillstreakWeapon.Value)
	{
		if (const auto& pLocal = g_EntityCache.GetLocal())
		{
			...
			const auto streaksResource = g_EntityCache.GetPR()->GetStreaks(I::EngineClient->GetLocalPlayer());
			...
			auto streaksPlayer = reinterpret_cast<NetworkVar_m_nStreaks*>(*reinterpret_cast<uintptr_t*>(reinterpret_cast<uintptr_t>(pLocal) + 1734 * sizeof(DWORD))); //C_TFPlayer CTFPlayerShared::m_nStreaks
			if (!streaksPlayer ||
				reinterpret_cast<uintptr_t>(streaksPlayer) < 0x1000 ||
				!streaksPlayer->m_Value ||
				reinterpret_cast<uintptr_t>(streaksPlayer->m_Value) < 0x1000)
			{
				return;
			}
