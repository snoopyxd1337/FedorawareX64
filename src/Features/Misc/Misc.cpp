...
void CMisc::RunPre(CUserCmd* pCmd, bool* pSendPacket)
{
	...
	{
		// Anti-warp is handled by TickHandler now. The old FastStop path fights movement prediction during tickshift.
...
void CMisc::RunMid(CUserCmd* pCmd, const EHANDLE& nOldGroundEnt)
{
	if (const auto& pLocal = g_EntityCache.GetLocal())
	{
...
void CMisc::PrintProjAngles(CBaseEntity* pLocal)
{
	...
	{
		if (I::ClientEntityList->GetClientEntityFromHandleSafe(pEntity->GethOwner()) != pLocal) { continue; }
...
void CMisc::EdgeJump(CBaseEntity* pLocal, CUserCmd* pCmd, const EHANDLE& nOldGroundEnt)
{
	if (!nOldGroundEnt.Get() || !Vars::Misc::EdgeJump.Value) { return; }
