...
namespace S
{
	MAKE_SIGNATURE(CTFPlayerSharedUtils_GetEconItemViewByLoadoutSlot, CLIENT_DLL, "48 89 6C 24 ? 56 41 54 41 55 41 56 41 57 48 83 EC", 0x0);
	MAKE_SIGNATURE(C_EconItemView_GetItemName, CLIENT_DLL, "40 53 48 83 EC ? 48 8B D9 C6 81 ? ? ? ? ? E8 ? ? ? ? 48 8B 8B", 0x0);
}
...
bool CESP::ShouldRun()
{
	...
	{
		FDBG_SKIP("Feature.ESP.ShouldRun", "clean screenshot active");
		return false;
	}

	if (!I::EngineClient->IsInGame() && I::EngineVGui->IsGameUIVisible())
	{
		FDBG_SKIP("Feature.ESP.ShouldRun", "game UI visible");
		return false;
	}
...
void CESP::Draw()
{
	FDBG_SCOPE("Feature.ESP.DrawBody");
	...
	if (const auto& pLocal = g_EntityCache.GetLocal())
	{
		...
		{
			FDBG_SKIP("Feature.ESP.DrawBody", "ShouldRun returned false");
		}
	}
	else
	{
		FDBG_SKIP("Feature.ESP.DrawBody", "local entity null");
	}
}
...
void CESP::DrawPlayers(CBaseEntity* pLocal)
{
	...
	{
		FDBG_SKIP("Feature.ESP.Players", "player ESP or main ESP inactive");
		return;
	}
	...
	if (!cResource)
	{
		FDBG_SKIP("Feature.ESP.Players", "player resource null");
		return;
	}
	...
	if (players.empty())
	{
		FDBG_SKIP("Feature.ESP.Players", "PLAYERS_ALL group empty");
		return;
	}
		...
		if (GetDrawBounds(player, vTrans, x, y, w, h))
		{
			...
			if (pWeapon)
			{
				...
				if (Vars::ESP::Players::WeaponText.Value)
				{
					...
					static auto getItemName = reinterpret_cast<const wchar_t* (__fastcall*)(void*)>(
...
void CESP::DrawBuildings(CBaseEntity* pLocal) const
{
	...
	{
		FDBG_SKIP("Feature.ESP.Buildings", "building ESP or main ESP inactive");
		return;
	}
	...
	const auto& buildings = g_EntityCache.GetGroup(Vars::ESP::Buildings::IgnoreTeammates.Value ? EGroupType::BUILDINGS_ENEMIES : EGroupType::BUILDINGS_ALL);
	if (buildings.empty())
	{
		FDBG_SKIP("Feature.ESP.Buildings", "building group empty");
		return;
	}
...
void CESP::DrawWorld() const
{
	...
	if (!pLocal)
	{
		FDBG_SKIP("Feature.ESP.World", "local entity null");
		return;
	}

	if (!Vars::ESP::World::Active.Value || !Vars::ESP::Main::Active.Value)
	{
		FDBG_SKIP("Feature.ESP.World", "world ESP or main ESP inactive");
		return;
	}
