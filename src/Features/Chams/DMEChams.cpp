...
namespace
{
	void PrepareModelMaterial(KeyValues* pKV)
	{
		if (!pKV)
		{
			return;
		...
		// Mark custom materials as model materials so VertexLit/Phong/Envmap paths are honored.
		pKV->SetString("$model", "1");
		pKV->SetString("$cloakpassenabled", "1");
	}

	IMaterial* CreateMaterialFromVMT(const char* materialName, const char* vmt)
	{
		if (!materialName || !vmt)
		{
			return nullptr;
		...
		auto kv = new KeyValues(materialName);
		if (!g_KeyValUtils.LoadFromBuffer(kv, materialName, vmt))
		{
			return nullptr;
		}

		PrepareModelMaterial(kv);
		return F::DMEChams.CreateNRef(materialName, kv);
	}
}
...
IMaterial* CDMEChams::CreateNRef(char const* szName, void* pKV, bool bSave)
{
	...
	if (!returnMaterial) { return nullptr; }
	returnMaterial->IncrementReferenceCount();
	RegisterMaterial(returnMaterial);
...
void CDMEChams::CreateMaterials()
{
	...
	v_MatList.push_back(CreateMaterialFromVMT("m_pMatShaded",
		"\"VertexLitGeneric\"\n"
		"{\n"
		...
		"}"));
	v_MatList.push_back(CreateMaterialFromVMT("m_pMatShiny",
		"\"VertexLitGeneric\"\n"
		"{\n"
		"\t\"$basetexture\" \"vgui/white_additive\"\n"
		"\t\"$bumpmap\" \"vgui/white_additive\"\n"
		...
		"\t\"$selfillumfresnelminmaxexp\" \"[-0.25 1 1]\"\n"
		"}"));
	v_MatList.push_back(CreateMaterialFromVMT("m_pMatFlat",
		"\"UnlitGeneric\"\n"
		"{\n"
		"\t\"$basetexture\" \"vgui/white_additive\"\n"
		"}"));
	v_MatList.push_back(CreateMaterialFromVMT("m_pMatWFShaded",
		"\"VertexLitGeneric\"\n"
		"{\n"
		"\t\"$wireframe\" \"1\"\n"
		"\t\"$basetexture\" \"vgui/white_additive\"\n"
		...
		"\t\"$selfillumfresnelminmaxexp\" \"[-0.25 1 1]\"\n"
		"}"));
	v_MatList.push_back(CreateMaterialFromVMT("m_pMatWFShiny",
		"\"VertexLitGeneric\"\n"
		"{\n"
		"\t\"$wireframe\" \"1\"\n"
		"\t\"$basetexture\" \"vgui/white_additive\"\n"
		...
		"\t\"$selfillumfresnelminmaxexp\" \"[-0.25 1 1]\"\n"
		"}"));
	v_MatList.push_back(CreateMaterialFromVMT("m_pMatWFFlat",
		"\"UnlitGeneric\"\n"
		"{\n"
		"\t\"$wireframe\" \"1\"\n"
		"\t\"$basetexture\" \"vgui/white_additive\"\n"
		"}"));
	v_MatList.push_back(CreateMaterialFromVMT("m_pMatFresnel",
		"\"VertexLitGeneric\"\n"
		"{\n"
		"\t\"$basetexture\" \"vgui/white_additive\"\n"
		...
		"\t\"$envmaptint\" \"[1 1 1]\"\n"
		"}"));
	v_MatList.push_back(CreateMaterialFromVMT("m_pMatBrick",
		"\"VertexLitGeneric\"\n"
		"{\n"
		...
		"\t\"$rimlightboost\" \"10\"\n"
		"}"));
	v_MatList.push_back(CreateMaterialFromVMT("m_pMatScuffed",
		"\"VertexLitGeneric\"\n"
		"{\n"
...
void CDMEChams::RegisterMaterial(IMaterial* pMaterial)
{
	if (!pMaterial) { return; }
	if (std::ranges::find(v_MatListGlobal, pMaterial) == v_MatListGlobal.end())
	{
		v_MatListGlobal.push_back(pMaterial);
	}
}

void CDMEChams::UnregisterMaterial(IMaterial* pMaterial)
{
	if (!pMaterial) { return; }
	v_MatListGlobal.erase(std::remove(v_MatListGlobal.begin(), v_MatListGlobal.end(), pMaterial), v_MatListGlobal.end());
	backupInformation.erase(pMaterial);
}
...
Chams_t getChamsType(int nIndex, CBaseEntity* pEntity = nullptr)
{
	switch (nIndex)
	{
		...
		case 4:
		{
			...
			if (CBaseEntity* pOwner = I::ClientEntityList->GetClientEntityFromHandleSafe(pEntity->m_hOwnerEntity()))
			{
		...
		{
			if (!pEntity) { return Chams_t(); }
			if (CBaseEntity* pOwner = I::ClientEntityList->GetClientEntityFromHandleSafe(pEntity->GetThrower()))
			{
				return GetPlayerChams(pOwner);
			}
			return Chams_t();
		...
		// handle
		{
			...
			if (pEntity && pLocal)
			{
				...
				if (drawType == 4 || drawType == 5){
					pOwner = I::ClientEntityList->GetClientEntityFromHandleSafe(pEntity->m_hOwnerEntity());
				}
