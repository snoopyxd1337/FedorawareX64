...
namespace
{
	std::vector<std::string> g_ProjectileDebugBuffer;

	void ProjectileDebugLog(const char* stage, const std::string& message)
	{
		...
		g_ProjectileDebugBuffer.push_back(std::format("{} | {}", stage, message));
	}

	void ProjectileDebugReset()
	{
		if (!Vars::Debug::DebugInfo.Value)
		{
			return;
		...
		g_ProjectileDebugBuffer.clear();
	}

	void ProjectileDebugFlush()
	{
		if (!Vars::Debug::DebugInfo.Value || g_ProjectileDebugBuffer.empty())
		{
			return;
		}

		Utils::ConLog("ProjAim", "----- shot debug begin -----", { 255, 180, 0, 255 }, true);
		for (const auto& line : g_ProjectileDebugBuffer)
		{
			Utils::ConLog("ProjAim", line.c_str(), { 255, 180, 0, 255 }, true);
		}
		Utils::ConLog("ProjAim", "----- shot debug end -----", { 255, 180, 0, 255 }, true);
		g_ProjectileDebugBuffer.clear();
	}

	struct ProjectileSetup_t
	{
		Vec3 offset = {};
		float redirectDistance = 0.0f;
		bool allowFlip = true;
	};

	bool GetProjectileSetup(CBaseEntity* pLocal, CBaseCombatWeapon* pWeapon, ProjectileSetup_t& out)
	{
		if (!pLocal || !pWeapon)
		{
			return false;
		...
		const bool bDucking = pLocal->m_fFlags() & FL_DUCKING;
		const bool bFlipped = g_ConVars.cl_flipviewmodels && g_ConVars.cl_flipviewmodels->GetBool();
		...
		switch (pWeapon->GetWeaponID())
		{
			...
			{
				out.offset = { 23.5f, 12.0f, bDucking ? 8.0f : -3.0f };
				if (G::CurItemDefIndex == Soldier_m_TheOriginal)
				{
					out.offset.y = 0.0f;
				}
				out.redirectDistance = 2000.0f;
				return true;
			}
			...
			{
				out.offset = { 23.5f, 8.0f, bDucking ? 8.0f : -3.0f };
				out.redirectDistance = 2000.0f;
				return true;
			}
			...
			{
				out.offset = { 70.0f, bFlipped ? -7.0f : 7.0f, -9.0f };
				out.allowFlip = false;
				out.redirectDistance = 0.0f;
				return true;
			}
			case TF_WEAPON_CLEAVER:
			{
				out.offset = { 32.0f, 0.0f, -15.0f };
				return true;
			}
			case TF_WEAPON_GRAPPLINGHOOK:
			{
				out.offset = { 23.5f, bFlipped ? -8.0f : 8.0f, -3.0f };
				out.allowFlip = false;
				out.redirectDistance = 2000.0f;
				return true;
			}
	...
	Vec3 GetProjectileSpawnPos(CBaseEntity* pLocal, const Vec3& viewAngles, const ProjectileSetup_t& setup)
	{
		Vec3 offset = setup.offset;
		if (setup.allowFlip && g_ConVars.cl_flipviewmodels && g_ConVars.cl_flipviewmodels->GetBool())
		{
			offset.y *= -1.0f;
		}
		...
		return pLocal->GetShootPos() + (forward * offset.x) + (right * offset.y) + (up * offset.z);
	}

	Vec3 GetCorrectedProjectileViewAngles(CBaseEntity* pLocal, CBaseCombatWeapon* pWeapon, const Vec3& projectileAngles, Vec3* pSpawnPos = nullptr)
	{
		ProjectileSetup_t setup = {};
		if (!GetProjectileSetup(pLocal, pWeapon, setup))
		{
			if (pSpawnPos)
			{
				*pSpawnPos = pLocal->GetShootPos();
			}
			return projectileAngles;
		}

		Vec3 viewAngles = projectileAngles;
		Vec3 spawnPos = pLocal->GetShootPos();

		for (int i = 0; i < 2; i++)
		{
			spawnPos = GetProjectileSpawnPos(pLocal, viewAngles, setup);
			if (setup.redirectDistance <= 0.0f)
			{
				viewAngles = projectileAngles;
				continue;
			}

			Vec3 projectileForward = {};
			Math::AngleVectors(projectileAngles, &projectileForward);
			const Vec3 endPos = spawnPos + projectileForward * setup.redirectDistance;
			viewAngles = Math::CalcAngle(pLocal->GetShootPos(), endPos);
		}

		if (pSpawnPos)
		{
			*pSpawnPos = GetProjectileSpawnPos(pLocal, viewAngles, setup);
		}
...
bool CAimbotProjectile::SolveProjectile(CBaseEntity* pLocal, CBaseCombatWeapon* pWeapon, CUserCmd* pCmd, Predictor_t& predictor, const ProjectileInfo_t& projInfo, Solution_t& out)
{
	...
	*/
	const bool useTPred = predictor.m_vVelocity.LengthSqr() > 1.0f || predictor.m_vAcceleration.LengthSqr() > 1.0f;
	ProjectileDebugLog(
		"Solve.Start",
		std::format(
			"ent={} vel=[{:.1f},{:.1f},{:.1f}] accel=[{:.1f},{:.1f},{:.1f}] cur=[{:.1f},{:.1f},{:.1f}] usePred={} rawEntVel=[{:.1f},{:.1f},{:.1f}] maxTime={:.3f}",
			predictor.m_pEntity ? predictor.m_pEntity->GetIndex() : -1,
			predictor.m_vVelocity.x, predictor.m_vVelocity.y, predictor.m_vVelocity.z,
			predictor.m_vAcceleration.x, predictor.m_vAcceleration.y, predictor.m_vAcceleration.z,
			predictor.m_vPosition.x, predictor.m_vPosition.y, predictor.m_vPosition.z,
			useTPred ? 1 : 0,
			predictor.m_pEntity ? predictor.m_pEntity->GetVecVelocity().x : 0.0f,
			predictor.m_pEntity ? predictor.m_pEntity->GetVecVelocity().y : 0.0f,
			predictor.m_pEntity ? predictor.m_pEntity->GetVecVelocity().z : 0.0f,
			maxTime));
	...
	if (!useTPred)
	{
		...
		{
			const Vec3 projectileAngles = { -RAD2DEG(out.m_flPitch), RAD2DEG(out.m_flYaw), 0.0f };
			Vec3 refinedLocalPos = vLocalPos;
			GetCorrectedProjectileViewAngles(pLocal, pWeapon, projectileAngles, &refinedLocalPos);
			if (!CalcProjAngle(refinedLocalPos, staticPos.value(), projInfo, out))
			{
				return false;
			}
		}
		...
		if (WillProjectileHit(pLocal, pWeapon, pCmd, staticPos.value(), out, projInfo, predictor))
		{
			...
			ProjectileDebugLog(
				"Solve.StaticHit",
				std::format(
					"pred=[{:.1f},{:.1f},{:.1f}] pitch={:.2f} yaw={:.2f} time={:.3f}",
					G::PredictedPos.x, G::PredictedPos.y, G::PredictedPos.z,
					-RAD2DEG(out.m_flPitch), RAD2DEG(out.m_flYaw), out.m_flTime));
			return true;
		}
	}
	else
	{
		...
		if (F::MoveSim.Initialize(predictor.m_pEntity))
		{
			...
			{
				if (!F::MoveSim.RunTick(moveData, absOrigin))
				{
					break;
				}
...

				{
					...
					Vec3 refinedLocalPos = vLocalPos;
					GetCorrectedProjectileViewAngles(pLocal, pWeapon, projectileAngles, &refinedLocalPos);
					if (!CalcProjAngle(refinedLocalPos, vPredictedPos, projInfo, out))
					{
						break;
					}
				}
				...
				if (WillProjectileHit(pLocal, pWeapon, pCmd, vPredictedPos, out, projInfo, predictor))
				{
					...
					ProjectileDebugLog(
						"Solve.DynamicHit",
						std::format(
							"tick={} pred=[{:.1f},{:.1f},{:.1f}] path=[{:.1f},{:.1f},{:.1f}] pitch={:.2f} yaw={:.2f} time={:.3f} latencyAdj={:.3f}",
							n,
							G::PredictedPos.x, G::PredictedPos.y, G::PredictedPos.z,
							absOrigin.x, absOrigin.y, absOrigin.z,
							-RAD2DEG(out.m_flPitch), RAD2DEG(out.m_flYaw), out.m_flTime, fLatency));
					return true;
				}
			}
			F::MoveSim.Restore();
										  ...
										  const Predictor_t& predictor)
{
	...
	const Vec3 projectileAngles = { -RAD2DEG(out.m_flPitch), RAD2DEG(out.m_flYaw), 0.0f };
	Vec3 vVisCheck = {};
	const Vec3 predictedViewAngles = GetCorrectedProjectileViewAngles(pLocal, pWeapon, projectileAngles, &vVisCheck);
	CGameTrace trace = {};
	static CTraceFilterWorldAndPropsOnly traceFilter = {};
	traceFilter.pSkip = predictor.m_pEntity;
...
bool CAimbotProjectile::VerifyTarget(CBaseEntity* pLocal, CBaseCombatWeapon* pWeapon, CUserCmd* pCmd, Target_t& target)
{
	...
	if (!SolveProjectile(pLocal, pWeapon, pCmd, predictor, projInfo, solution))
	{
		ProjectileDebugLog(
			"Verify.Fail",
			std::format(
				"ent={} cur=[{:.1f},{:.1f},{:.1f}] vel=[{:.1f},{:.1f},{:.1f}]",
				target.m_pEntity ? target.m_pEntity->GetIndex() : -1,
				target.m_vPos.x, target.m_vPos.y, target.m_vPos.z,
				vVelocity.x, vVelocity.y, vVelocity.z));
		return false;
	}

	target.m_vAngleTo = GetCorrectedProjectileViewAngles(pLocal, pWeapon, { -RAD2DEG(solution.m_flPitch), RAD2DEG(solution.m_flYaw), 0.0f });
	ProjectileDebugLog(
		"Verify.Ok",
		std::format(
			"ent={} cur=[{:.1f},{:.1f},{:.1f}] pred=[{:.1f},{:.1f},{:.1f}] finalAng=[{:.2f},{:.2f},{:.2f}] travel={:.3f}",
			target.m_pEntity ? target.m_pEntity->GetIndex() : -1,
			target.m_vPos.x, target.m_vPos.y, target.m_vPos.z,
			G::PredictedPos.x, G::PredictedPos.y, G::PredictedPos.z,
			target.m_vAngleTo.x, target.m_vAngleTo.y, target.m_vAngleTo.z,
			solution.m_flTime));
...
bool CAimbotProjectile::GetSplashTarget(CBaseEntity* pLocal, CBaseCombatWeapon* pWeapon, CUserCmd* pCmd, Target_t& outTarget)
{
	...
	splashRadius = Utils::ATTRIB_HOOK_FLOAT(148, "mult_explosion_radius", pWeapon, 0, true);
...
void CAimbotProjectile::Run(CBaseEntity* pLocal, CBaseCombatWeapon* pWeapon, CUserCmd* pCmd)
{
	...
	if (GetTarget(pLocal, pWeapon, pCmd, target) || GetSplashTarget(pLocal, pWeapon, pCmd, target))
	{
		...
		const Vec3 currentAngle = Math::CalcAngle(pLocal->GetShootPos(), target.m_vPos);
		const Vec3 predictedAngle = Math::CalcAngle(pLocal->GetShootPos(), G::PredictedPos);
		ProjectileDebugLog(
			"Run.Target",
			std::format(
				"ent={} cur=[{:.1f},{:.1f},{:.1f}] pred=[{:.1f},{:.1f},{:.1f}] curAng=[{:.2f},{:.2f}] predAng=[{:.2f},{:.2f}] sendAng=[{:.2f},{:.2f},{:.2f}] dAng=[{:.2f},{:.2f}] attack={} weaponCan={}",
				target.m_pEntity ? target.m_pEntity->GetIndex() : -1,
				target.m_vPos.x, target.m_vPos.y, target.m_vPos.z,
				G::PredictedPos.x, G::PredictedPos.y, G::PredictedPos.z,
				currentAngle.x, currentAngle.y,
				predictedAngle.x, predictedAngle.y,
				target.m_vAngleTo.x, target.m_vAngleTo.y, target.m_vAngleTo.z,
				target.m_vAngleTo.x - currentAngle.x, target.m_vAngleTo.y - currentAngle.y,
				IsAttacking(pCmd, pWeapon) ? 1 : 0,
				G::WeaponCanAttack ? 1 : 0));
		...
		if (Vars::Aimbot::Projectile::AimMethod.Value == 1)
		{
			if (IsFlameThrower)
			{
				...
				ProjectileDebugLog(
					"Run.PostAim",
					std::format(
						"cmdAng=[{:.2f},{:.2f},{:.2f}] buttons=0x{:X} silent={}",
						pCmd->viewangles.x, pCmd->viewangles.y, pCmd->viewangles.z,
						pCmd->buttons, G::SilentTime ? 1 : 0));
			}
