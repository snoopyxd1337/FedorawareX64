...
namespace
{
	bool RunMovementSimulation(CBaseEntity* pPlayer, CMoveData* pMoveData)
	{
		if (!pPlayer || !pMoveData || !I::TFGameMovement)
		{
			return false;
		}
		...
		__try
		{
			I::TFGameMovement->ProcessMovement(pPlayer, pMoveData);
			return true;
		}
...
{
	if (!I::EngineClient || !I::EngineClient->IsInGame() || !I::Prediction || !I::GlobalVars || !I::TFGameMovement
		|| !pPlayer || !pPlayer->IsPlayer() || !pPlayer->IsAlive() || pPlayer->GetDormant() || !pPlayer->IsInValidTeam() || pPlayer->deadflag())
	{
		return false;
	}
	...
	//the hacks that make it work
	{
		...
		{
			pPlayer->m_hGroundEntity() = EHANDLE(); //without this nonlocal players get snapped to the floor
		}
...
bool CMovementSimulation::StrafePrediction()
{
	...
	{			//	we've already done the math, don't do it again
		...
		const int velocityRecordCount = static_cast<int>(mVelocityRecord.size());
		if (velocityRecordCount < 1)
		{
			return false;
		}

		const int iSamples = std::min(Vars::Aimbot::Projectile::StrafePredictionSamples.Value, velocityRecordCount);
...
bool CMovementSimulation::RunTick(CMoveData& moveDataOut, Vec3& m_vecAbsOrigin)
{
	...
	if (!I::EngineClient || !I::EngineClient->IsInGame() || !I::Prediction || !I::GlobalVars || !I::TFGameMovement
		|| m_pPlayer->GetClassID() != ETFClassID::CTFPlayer || !m_pPlayer->IsAlive() || m_pPlayer->GetDormant())
	{
		bDontPredict = true;
		return false;
	}
	...
	if (!RunMovementSimulation(m_pPlayer, &m_MoveData))
	{
		bDontPredict = true;
		return false;
	}
