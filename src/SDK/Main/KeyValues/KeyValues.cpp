...
bool CKeyValUtils::LoadFromBuffer(KeyValues* key_value, char const* resource_name, const char* buffer, void* file_system, const char* path_id)
{
	...
	return loadFromBuffer ? loadFromBuffer(key_value, resource_name, buffer, file_system, path_id) : false;
}
...
KeyValues* CKeyValUtils::Initialize(KeyValues* key_value, char* name)
{
	...
	return initialize ? initialize(key_value, name) : nullptr;
}
...
void KeyValues::Initialize(char* name)
{
	using FN = KeyValues * (__thiscall*)(KeyValues*, char*);
	static FN initialize = reinterpret_cast<FN>(S::KeyValUtils_Initialize());
	if (initialize) { initialize(this, name); }
}
...
KeyValues* KeyValues::FindKey(const char* keyName, bool bCreate)
{
	...
	return findKey ? findKey(this, keyName, bCreate) : nullptr;
}
...
const wchar_t* KeyValues::GetWString(const char* keyName, const wchar_t* defaultValue)
{
	...
	if (dat)
	{
		...
		switch (dat->m_iDataType)
		{
			...
			case TYPE_STRING:
			{
				int bufSize = static_cast<int>(strlen(dat->m_sValue)) + 1;
