...
bool CEntityCache::Fill()
{
	if (!I::ClientEntityList || !I::EngineClient || !I::EngineClient->IsInGame())
	{
		return false;
	}
	...
	if (localIndex <= 0 || localIndex > maxClients)
	{
		return false;
	}

	CBaseEntity* pLocal = I::ClientEntityList->GetClientEntitySafe(localIndex);
	if (!pLocal || pLocal->GetClassID() != ETFClassID::CTFPlayer || !pLocal->IsInValidTeam())
	{
		return false;
	}

	m_pLocal = pLocal;
	m_pLocalWeapon = m_pLocal->IsAlive() ? m_pLocal->GetActiveWeapon() : nullptr;
	...
	switch (m_pLocal->GetObserverMode())
	{
		...
		{
			m_pObservedTarget = I::ClientEntityList->GetClientEntityFromHandleSafe(m_pLocal->GetObserverTarget());
			break;
		}
		default: break;
	}
	...
	// Cache entities
	const int highestEntity = std::clamp(I::ClientEntityList->GetHighestEntityIndex(), 1, 4096);
	for (int n = 1; n < highestEntity; n++)
	{
		...
		case ETFClassID::CTFProjectile_SpellFireball:
			{
				...
				if (I::ClientEntityList->GetClientEntityFromHandleSafe(pEntity->GetThrower()) == m_pLocal ||
					I::ClientEntityList->GetClientEntityFromHandleSafe(pEntity->GethOwner()) == m_pLocal) {
			...
			{
				if (I::ClientEntityList->GetClientEntityFromHandleSafe(pEntity->GetTargetPlayer()) == m_pLocal)
