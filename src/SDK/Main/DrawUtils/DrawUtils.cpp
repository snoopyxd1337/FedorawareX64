...
namespace S
{
	MAKE_SIGNATURE(CDraw_GetIcon, CLIENT_DLL, "40 53 48 81 EC ? ? ? ? 48 8B DA", 0x0);
}
...
void CDraw::String(const Font_t& font, int x, int y, const Color_t& clr, const EStringAlign& align, const char* str, ...)
{
	...
	I::VGuiSurface->DrawPrintText(wstr, static_cast<int>(wcslen(wstr)));
}
...
{
	using FN = CHudTexture* (__fastcall*)(void*, const char*, int);
	static auto fnGetIcon = S::CDraw_GetIcon.As<FN>();
	return fnGetIcon ? fnGetIcon(nullptr, szIcon, eIconFormat) : nullptr;
}
