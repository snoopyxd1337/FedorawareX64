...
int CBaseCombatWeapon::GetBulletAmount()
{
	...
	const auto v7 = *reinterpret_cast<DWORD*>(base + 2864);
	const auto v8 = *reinterpret_cast<DWORD*>(base + 2852) << 6;
	int bulletspershot = *reinterpret_cast<DWORD*>(static_cast<uintptr_t>(v8) + v7 + 1788);
	if (bulletspershot >= 1)
	{
