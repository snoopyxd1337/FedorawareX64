...
namespace S
{
	MAKE_SIGNATURE(ATTRIB_HOOK_FLOAT, CLIENT_DLL, "4C 8B DC 49 89 5B ? 49 89 6B ? 56 57 41 54 41 56 41 57 48 83 EC ? 48 8B 3D ? ? ? ? 4C 8D 35", 0x0);
	MAKE_SIGNATURE(CTE_DispatchEffect, CLIENT_DLL, "48 89 5C 24 ? 57 48 83 EC ? 48 8B D9 48 8B FA 48 8D 4C 24", 0x0);
	MAKE_SIGNATURE(GetParticleSystemIndex, CLIENT_DLL, "40 53 48 83 EC ? 48 8B D9 48 85 C9 74 ? 48 8B 0D ? ? ? ? 48 8B D3 48 8B 01 FF 50 ? 3D", 0x0);
	MAKE_SIGNATURE(UTIL_ParticleTracer, CLIENT_DLL, "48 89 5C 24 ? 48 89 74 24 ? 48 89 7C 24 ? 4C 89 74 24 ? 55 48 8D 6C 24 ? 48 81 EC ? ? ? ? 49 63 F1", 0x0);
}
...
namespace Utils
{
	...
	__inline HWND GetGameWindow()
	{
		static HWND hwGame = nullptr;

		auto isValidWindow = [](HWND window)
		{
			DWORD processId = 0;
			GetWindowThreadProcessId(window, &processId);
			return processId == GetCurrentProcessId() && IsWindowVisible(window) && GetWindow(window, GW_OWNER) == nullptr;
		};

		if (hwGame && IsWindow(hwGame) && isValidWindow(hwGame))
		{
			return hwGame;
		}

		hwGame = FindWindowA(nullptr, "Team Fortress 2");
		if (hwGame && isValidWindow(hwGame))
		{
			return hwGame;
		}
		...
		EnumWindows([](HWND window, LPARAM param) -> BOOL
		{
			...
			return TRUE;
		}, reinterpret_cast<LPARAM>(&hwGame));
	...
	{
		const HWND hwGame = GetGameWindow();

		return !hwGame || GetForegroundWindow() == hwGame;
	}
	...
	__inline std::wstring ConvertUtf8ToWide(const std::string_view& str)
	{
		...
		const int count = MultiByteToWideChar(CP_UTF8, 0, str.data(), strLength, nullptr, 0);
		std::wstring wstr(count, 0);
		MultiByteToWideChar(CP_UTF8, 0, str.data(), strLength, wstr.data(), count);
		return wstr;
	}
	...
	__inline float ATTRIB_HOOK_FLOAT(float baseValue, const char *searchString, CBaseEntity *ent, void *buffer, bool isGlobalConstString)
	{
		...
		return fn ? fn(baseValue, searchString, ent, buffer, isGlobalConstString) : baseValue;
	}
	...
	__inline int SeedFileLineHash(int seedvalue, const char *sharedname, int additionalSeed)
	{
		...
		CRC32_ProcessBuffer(&retval, sharedname, static_cast<int>(strlen(sharedname)));
	...
	__inline int GetWeaponByID(CBaseEntity* pPlayer, int pWeaponID)
	{
		...
		const auto hWeapons = pPlayer->GetMyWeaponHandles();
		// Go through the handle array and search for the item
		for (int i = 0; i < MAX_WEAPONS; i++)
		{
			if (!hWeapons[i].IsValid())
			{
				continue;
			}

			// Get the weapon
			auto* weapon = reinterpret_cast<CBaseCombatWeapon*>(hWeapons[i].Get());
...
namespace Particles {
	...
	inline int GetParticleSystemIndex(const char* pParticleSystemName)
	{
		...
		return fnGetParticleSystemIndex ? fnGetParticleSystemIndex(pParticleSystemName) : -1;
	}
